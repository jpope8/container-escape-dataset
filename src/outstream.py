...
class OutStream:
    ...
    def __init__(self, f=None, append=False):
        """
        Construct self to wrap around a stream. If f is provided, then
        the stream is a file of that name.  Otherwise the stream is
        standard output.  If append is True, file is appended to,
        otherwise overwrite any existing content.
        """
        if f is None:
            ...
            if sys.hexversion < 0x03000000:
                self._stream = open(f, modifier)
            else:
                self._stream = open(f, modifier, encoding='utf-8')
